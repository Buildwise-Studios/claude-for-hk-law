...
- `/litigation-legal:matter-workspace close <slug>` — archive a matter (move to `~/.claude/plugins/config/claude-for-hk-law/litigation-legal/matters/_archived/`, never delete)
...
3. Dispatch on the first token of `$ARGUMENTS`:
   - `new` → run the intake interview, write `~/.claude/plugins/config/claude-for-hk-law/litigation-legal/matters/<slug>/matter.md`, seed `history.md` and `notes.md`.
   - `list` → enumerate `~/.claude/plugins/config/claude-for-hk-law/litigation-legal/matters/*/matter.md`, print a table, mark the active matter.
   - `switch` → update the `Active matter:` line in the practice-level CLAUDE.md.
   - `close` → move `~/.claude/plugins/config/claude-for-hk-law/litigation-legal/matters/<slug>/` to `~/.claude/plugins/config/claude-for-hk-law/litigation-legal/matters/_archived/<slug>/`, log the close date in `history.md`.
...
```
~/.claude/plugins/config/claude-for-hk-law/litigation-legal/
