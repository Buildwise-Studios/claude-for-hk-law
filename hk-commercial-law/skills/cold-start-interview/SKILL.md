...
description: >
  Run the cold-start interview to configure your Hong Kong commercial law practice profile.
  Use on first install, when the practice profile is missing or has [PLACEHOLDER] markers,
  or when the user says "set up the plugin" or "configure hk-commercial-law".
argument-hint: "[--redo to re-run]"
---

# /cold-start-interview

Writes `~/.claude/plugins/config/claude-for-hk-law/hk-commercial-law/CLAUDE.md`.

## Instructions

1. Read `~/.claude/plugins/config/claude-for-hk-law/hk-commercial-law/CLAUDE.md`. If populated and `--redo` not passed, confirm before overwriting.
