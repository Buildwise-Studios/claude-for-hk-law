...
description: >
  Run the cold-start interview to configure your Hong Kong Basic Law practice profile.
  Use on first install, when the practice profile is missing or has [PLACEHOLDER] markers,
  or when the user says "set up the plugin" or "configure hk-basic-law".
argument-hint: "[--redo to re-run]"
---

# /cold-start-interview

Writes `~/.claude/plugins/config/claude-for-hk-law/hk-basic-law/CLAUDE.md`.

## Instructions

1. Read `~/.claude/plugins/config/claude-for-hk-law/hk-basic-law/CLAUDE.md`. If populated and `--redo` not passed, confirm before overwriting.
