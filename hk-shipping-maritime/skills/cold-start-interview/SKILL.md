...
description: >
  Run the cold-start interview to configure your Hong Kong shipping and maritime practice profile.
  Use on first install, when the practice profile is missing or has [PLACEHOLDER] markers,
  or when the user says "set up the plugin" or "configure hk-shipping-maritime".
argument-hint: "[--redo to re-run]"
---

# /cold-start-interview

Writes `~/.claude/plugins/config/claude-for-hk-law/hk-shipping-maritime/CLAUDE.md`.

## Instructions

1. Read `~/.claude/plugins/config/claude-for-hk-law/hk-shipping-maritime/CLAUDE.md`. If populated and `--redo` not passed, confirm before overwriting.
